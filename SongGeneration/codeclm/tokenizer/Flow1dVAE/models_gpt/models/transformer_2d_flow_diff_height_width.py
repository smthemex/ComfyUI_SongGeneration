...
from diffusers.models.embeddings import get_2d_sincos_pos_embed, PixArtAlphaTextProjection
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.base_size = patch_size[0] * patch_size[1]
        self.interpolation_scale = interpolation_scale
        pos_embed = get_2d_sincos_pos_embed(
            embed_dim, (height // patch_size[0], width // patch_size[1]), base_size=1, interpolation_scale=self.interpolation_scale
        )
...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    Parameters:
        ...
        sample_size (`list`, *optional*): The width of the latent images (specify if the input is **discrete**).
    ...
    def __init__(
        ...
        sample_size: Optional[list] = None,
        ...
        if self.is_input_continuous:
            ...
            self.patch_size = patch_size
            interpolation_scale = 1  # => 64 (= 512 pixart) has interpolation scale 1
        ...
        if self.is_input_continuous:
            ...
            self.proj_out_2 = nn.Linear(inner_dim, patch_size[0] * patch_size[1] * self.out_channels)
        elif self.is_input_patches and norm_type == "ada_norm_single":
            self.norm_out = nn.LayerNorm(inner_dim, elementwise_affine=False, eps=1e-6)
            self.scale_shift_table = nn.Parameter(torch.randn(2, inner_dim) / inner_dim**0.5)
            self.proj_out = nn.Linear(inner_dim, patch_size[0] * patch_size[1] * self.out_channels)
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            height, width = hidden_states.shape[-2] // self.patch_size[0], hidden_states.shape[-1] // self.patch_size[1]
        ...
        if self.is_input_patches:
            ...
            hidden_states = hidden_states.reshape(
                shape=(-1, height, width, self.patch_size[0], self.patch_size[1], self.out_channels)
            )
            hidden_states = torch.einsum("nhwpqc->nchpwq", hidden_states)
            output = hidden_states.reshape(
                shape=(-1, self.out_channels, height * self.patch_size[0], width * self.patch_size[1])
            )
